...
resx = xsize/X;
resrat = resx/resz;
lp = 0.4;                   % Built fraction
lv = 0.1;                   % Vegetated fraction
lfr = 0.4;                  % Frontal aspect ratio we want 
...
ngreen = 1;                 % Number of green spaces
...
paddingg = 2;               % Minimum gap between green space and anything elseing the original approach
area   = X*Y;
builtarea = lp*area;
greenarea = lv*area;
front = lf*area;
...
% Add the green
[green_list, second_topo,green_outline] = placer(ptchs_tall,paddingg,X,Y,first_topo,first_list);
plotter([],green_list,X,Y,padding,1,0);
%% Add the buildings and reshape them to match lamp
new_blocks = blocks_based_on_green(X,Y,padding,green_list,ngreen);
plotter(new_blocks,green_list,X,Y,padding,2,0);
% heights1 = heightadder1(new_blocks,minz);
% new_blocks1 = new_blocks;
% new_blocks1(:,6) = heights1;
%% write out this set of blocks
% final_list1 = [new_blocks1;green_list];
% final_list1(final_list1(:,6) == 1,6) = minz+1; 
% buildings1 = final_list1;

% buildings1(:,1) = buildings1(:,1)+1;
% buildings1(:,3) = buildings1(:,3)+1;
% green_outline(:,1) = green_outline(:,1)+1;
% green_outline(:,3) = green_outline(:,3)+1;
% buildings1(:,6) = buildings1(:,6)-1;
% buildings = buildings1; 
% blocks_file_path1 = ['/media/chris/Project1/uDALES_veg/experiments/' expnr1 '/buildings.' expnr1 '.mat'];
% outline_file_path1 = ['/media/chris/Project1/uDALES_veg/experiments/' expnr1 '/green_oultine.' expnr1 '.mat'];
% % params_file_path = ['/media/chris/Project1/uDALES_veg/experiments/' expnr '/generated_params.' expnr '.mat'];
% area_file_path1 = ['/media/chris/Project1/uDALES_veg/experiments/' expnr1 '/area.' expnr1 '.mat'];
% save(blocks_file_path1, 'buildings');
% save(outline_file_path1, 'green_outline');
% save(area_file_path1,'lams', 'area');

%% proceed as before, allowing changes
changed_blocks = changing_lamp(new_blocks, lp,area,minx,miny,padding, green_list, X,Y);
...
changed_blocks(:,6) = final_heights+1; % Add 1 to heights because the buildings start at z = 1 not z = 0
...
%% Get real area of total sim for the preprocessing
act_area = xsize*ysize*xtiles*ytiles; 

%% Write out the geometries for uDALES to use
% blocks_file_path = ['/media/chris/Project1/uDALES_veg/experiments/' expnr2 '/buildings.' expnr2 '.mat'];
% outline_file_path = ['/media/chris/Project1/uDALES_veg/experiments/' expnr2 '/green_oultine.' expnr2 '.mat'];
% % params_file_path = ['/media/chris/Project1/uDALES_veg/experiments/' expnr '/generated_params.' expnr '.mat'];
% area_file_path = ['/media/chris/Project1/uDALES_veg/experiments/' expnr2 '/area.' expnr2 '.mat'];
% save(blocks_file_path, 'buildings');
% save(outline_file_path, 'green_outline');
% % save(params_file_path,'lams');
% save(area_file_path,'lams', 'area');
...
function new_heights = heightadder1(blocks, minz)
    heights = minz*ones(length(blocks(:,1)),1);
    new_heights = heights;
end 
...
function new_blocks = changing_lamp(blocks,lp,Area, minx,miny,padding, green_list,X,Y,stage)
                ...
                for i = length(blocks(:,1)):-1:1
                    if or(blocks(i,2)- blocks(i,1)<minx, blocks(i,4)- blocks(i,3)<miny) 
                        ...
                        if Area1 <= remaining
                            ...
                            blocks(i,:) = [];
                            plotter(blocks,green_list,X,Y,padding,stage,it)
                            it = it + 1; 
...
function plotter(bs,gs,xlim,ylim,padding,stage,it)
    if stage == 1
        xl = 0; 
        xu = xlim;
        yl = 0;
        ...
        fh = figure(1)
        hold on
        light('Position', [90, 90, 35], 'Style', 'infinite');
        clr = [0.75,0.75,0.75];
    ...
    if stage == 2
        ...
        filename = ['pic' num2str(length(gs(:,1))+1) '.jpg'];
        saveas(fh,filename);
    end
    ...
    if stage == 3
        ...
        pause(0.5)
        filename = ['pic' num2str(length(gs(:,1))+1+it) '.jpg'];
        saveas(fh,filename);
    end
